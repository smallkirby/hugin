...
pub fn init(self: *Self, avail: mem.PhysRegion, reserveds: []mem.PhysRegion, log_fn: ?hugin.LogFn) void {
    rtt.expectEqual(0, avail.addr & mem.page_mask_4kib);
    rtt.expectEqual(0, avail.size & mem.page_mask_4kib);

    // Sort `reserveds` by the starting address.
    {
        const sort_fn = struct {
            fn lt(_: void, lhs: mem.PhysRegion, rhs: mem.PhysRegion) bool {
                return lhs.addr < rhs.addr;
            }
        }.lt;
        std.mem.sort(mem.PhysRegion, reserveds, {}, sort_fn);
    }

    // Feed the available region to the arena,
    {
        var cur = avail.addr;
        const end = avail.addr + avail.size;

        for (reserveds) |r| {
            const reserved_start = r.addr;
            const reserved_end = reserved_start + hugin.bits.roundup(r.size, mem.page_size);
            rtt.expectEqual(0, reserved_start & mem.page_mask_4kib);
...
fn allocPages(ctx: *anyopaque, num_pages: usize) Error![]align(mem.size_4kib) u8 {
    const self: *Self = @ptrCast(@alignCast(ctx));
    self.lock.lock();
    defer self.lock.unlock();

    return self.arena.allocPages(num_pages);
}
...
fn freePagesRaw(ctx: *anyopaque, addr: Virt, num_pages: usize) Error!void {
    ...
    self.arena.addRegion(addr, addr + num_pages * mem.size_4kib);
}
...
fn debugPrintStatistics(self: *Self, log_fn: hugin.LogFn) void {
...
fn rttTestBuddyAllocator(buddy_allocator: *Self) void {
    if (!hugin.is_runtime_test) return;
    ...
    {
        const page = allocator.allocPages(3) catch {
...
fn rttAllocatePage(list: *TestingAllocatedList, allocator: PageAllocator) []align(mem.size_4kib) u8 {
    const page = allocator.allocPages(1) catch {
