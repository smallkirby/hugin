...
pub const Vtable = struct {
    allocPages: *const fn (ctx: *anyopaque, num_pages: usize) Error![]align(mem.size_4kib) u8,
    freePages: *const fn (ctx: *anyopaque, slice: []u8) void,
    freePagesRaw: *const fn (ctx: *anyopaque, addr: hugin.mem.Virt, num_pages: usize) Error!void,
};

/// Allocate the given number of pages.
pub fn allocPages(self: Self, num_pages: usize) Error![]align(mem.size_4kib) u8 {
...
pub fn freePagesRaw(self: Self, addr: hugin.mem.Virt, num_pages: usize) Error!void {
    if (addr % hugin.mem.size_4kib != 0) return Error.InvalidRegion;
